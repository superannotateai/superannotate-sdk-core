...
class AnnotationRepository(BaseRepositry):
    ...
    def sort_annotatoins_by_size(self, project_id: int, folder_id: int,
                                 item_ids: List[int]) -> SortedAnnotationsResponse:
        ...
        response = self._session.request(
            ...
            json={"project_id": project_id, "item_ids": item_ids},  # todo add "folder_id": folder_id,
            build_url=False
        )
        response.raise_for_status()
        data = response.json()
        response_data["small"] = [
            i["data"] for i in data.get("small", {}).values()
        ]
        response_data["large"] = data.get("large", [])
        return response_data

    async def list_annotations(self, project_id: int, folder_id: int, item_ids: Iterable[int], callback: Callable = None) -> \
            List[
                dict]:
        query_params = {
    ...
    async def _sync_large_annotation(self, project_id: int, folder_id: int, item_id: int):
        sync_params = {
            "team_id": self._session.team_id,
            "project_id": project_id,
            "folder_id": folder_id,
    ...
    async def get_large_annotation(self, project_id: int, folder_id: int, item_id: int) -> dict:
        url = urljoin(self._session.assets_provider_url, self.URL_GET_ANNOTATIONS)
        query_params = {
            "project_id": project_id,
            "folder_id": folder_id,
    ...
    async def download_small_annotations(
            self,
            project_id: int, folder_id: int, item_ids: List[int], download_path: str, callback: Callable = None
    ):
        query_params = {
            "project_id": project_id,
            "folder_id": folder_id,
