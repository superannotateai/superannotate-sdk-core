...
from superannotate_core.infrastructure.repositories.item_repository import AttachmentMeta
...
from superannotate_core.core.exceptions import SAValidationException, SAInvalidInput, SAException
...
class Item(BaseItemEntity):
    ...
    @classmethod
    def get_by_id(cls, session: Session, project_id: int, folder_id, item_id: int) -> 'Item':
    ...
    @classmethod
    def get(cls, session, project_id: int, pk: Union[str, int], folder_id: int = None, include_custom_metadata=False):
    ...
    def list(
        ...
        repo = ItemRepository(session)
        if item_ids:
            _items = repo.list_by_ids(project_id=project_id, folder_id=folder_id, ids=item_ids)
        elif item_names:
            _items = repo.list_by_names(project_id=project_id, folder_id=folder_id, names=item_names)
        else:
            base_condition = Condition('project_id', project_id, EQ) & Condition('folder_id', folder_id, EQ)
            if condition:
    ...
    @classmethod
    async def alist_annotations(cls, session: Session, project_id: int, folder_id: int, items: List['Item']):
        ...
        large_items: List[BaseItemEntity] = list(filter(lambda item: item.id in large_item_ids, items))
        if large_items:
            for chunk in chunkify(large_items, max(session.MAX_COROUTINE_COUNT // 2, 2)):
    ...
    @classmethod
    async def aget_large_annotation(cls, session: Session, project_id: int, folder_id: int, item_id: int):
        repo = AnnotationRepository(session)
        return repo.get_large_annotation(project_id=project_id, folder_id=folder_id, item_id=item_id)
    ...
    def copy_items_by_names(
        ...
        skipped_item_namesa = ItemRepository(session).bulk_copy_by_names(
            ...
            destination_folder_id=destination_folder_id, item_names=items,
    ...
    @classmethod
    def bulk_set_annotation_status(cls, session: Session, project_id: int, folder_id: int,
                                   annotation_status: AnnotationStatus, items: List[str] = None):
        ItemRepository(session).set_statuses(project_id=project_id, folder_id=folder_id,
                                             annotation_status=annotation_status, item_names=items)

    @classmethod
    def bulk_set_approval_status(cls, session: Session, project_id: int, folder_id: int,
                                 approval_status: ApprovalStatus, items: List[str] = None):
        ItemRepository(session).set_approval_statuses(project_id=project_id, folder_id=folder_id,
                                                      approval_status=approval_status, item_names=items)
...
class AnnotationClass(AnnotationClassEntity):
    @classmethod
    def bulk_create(cls, session: Session, project_id: int, annotation_classes: List['AnnotationClass']):
        try:
            _annotation_classes = AnnotationClassesRepository(session).bulk_create(project_id, annotation_classes)
...
class Folder(FolderEntity):
    ...
    def get_item(self, pk: Union[str, int], include_custom_metadata=False):
        ...
        return _item.get(self.session, project_id=self.project_id, folder_id=self.id, pk=pk,
    ...
    def list_items(
        ...
        _item = PROJECT_ITEM_MAP[self.project.type]
        return _item.list(self.session, project_id=self.project_id, folder_id=self.id, condition=condition,
                          item_ids=item_ids, item_names=item_names)

    def get_annotations(self, *, condition: Condition = None, item_ids: List[int] = None,
                        item_names: List[str] = None) -> List[dict]:
        items = self.list_items(condition=condition, item_ids=item_ids, item_names=item_names)
        annotations = []
        if items:
            annotations = run_async(Item.alist_annotations(
        ...
        if item_names:
            ...
            annotations = list(sorted(annotations, key=lambda x: name_to_index[x["metadata"]["name"]]))
        return annotations

    def copy_items_by_name(self, destination_folder_id: int, items: List[str] = None, include_annotations=True) -> \
            List[str]:
    ...
    def move_items_by_name(self, destination_folder_id: int, items: List[str] = None) -> List[str]:
    ...
    def set_items_annotation_statuses(self, items: List[str], annotation_status: AnnotationStatus):
        Item.bulk_set_annotation_status(session=self.session, project_id=self.project_id, folder_id=self.id,
                                        items=items, annotation_status=annotation_status)

    def set_items_approval_statuses(self, items: List[str], approval_status: ApprovalStatus):
        Item.bulk_set_approval_status(session=self.session, project_id=self.project_id, folder_id=self.id,
                                      items=items, approval_status=approval_status)
    ...
    def _clean_folder_name(cls, val: str):
        ...
        if len(intersection) > 0:
            val = "".join(
                "_"
                if char in cls.Meta.SPECIAL_CHARACTERS
                else char
                for char in val
            )
            logger.warning(
    ...
    def get(cls, session: Session, project_id: int, pk: Union[str, int]):
        if isinstance(pk, int):
            return cls._from_entity(FolderRepository(session).get_by_id(project_id=project_id, folder_id=pk))
        elif isinstance(pk, str):
            condition = Condition('project_id', project_id, EQ) & Condition('namne', pk, EQ)
            repo = FolderRepository(session)
            folder = next((cls._from_entity(i) for i in repo.list(condition) if i.name == pk), None)
            if not folder:
                raise SAInvalidInput(f"Folder not found.")
            return folder
        else:
            raise SAInvalidInput("Invalid primery key.")
    ...
    def get_by_id(cls, session: Session, project_id: int, folder_id: int):
        return cls._from_entity(FolderRepository(session).get_by_id(project_id=project_id, folder_id=folder_id))

    # todo delete
    @classmethod
    def get_by_name(cls, session: Session, project_id: int, name: str) -> 'Folder':
        condition = Condition('project_id', project_id, EQ) & Condition('namne', name, EQ)
        repo = FolderRepository(session)
        folder = next((cls._from_entity(i) for i in repo.list(condition) if i.name == name), None)
        if not folder:
            raise SAInvalidInput(f"Folder not found.")
        return folder
    ...
    def delete_folders(cls, session: Session, project_id: int, names: List[str]) -> int:
        ...
        if to_delete:
            FolderRepository(session).bulk_delete(project_id=project_id, folder_ids=to_delete)
...
class Project(ProjectEntity):
    ...
    def get_item(self, pk: Union[str, int], include_custom_metadata=False):
        ...
        return _item.get(self.session, project_id=self.id, pk=pk,
                         include_custom_metadata=include_custom_metadata)

    @classmethod
    def create(cls, session, **data: Unpack[ProjectEntity]) -> 'Project':
        return cls._from_entity(ProjectRepository(session).create(ProjectEntity(**data)))

    def create_folder(self, name: str) -> Folder:
        return Folder.create(self.session, project_id=self.id, name=name)

    def delete_folders(self, folder_names: List[str]) -> int:
        return Folder.delete_folders(self.session, project_id=self.id, names=folder_names)
    ...
    def create_annotation_class(self, name: str, class_type: ClassTypeEnum, color: str,
